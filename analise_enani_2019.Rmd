---
title: "Análise Exploratória do ENANI 2019: Padrões Alimentares e Nutricionais Infantis no Brasil"
...
Este projeto utilizará o **ENANI 2019** (Estudo Nacional de Alimentação e Nutrição Infantil) [@enani2019], um conjunto de dados representativo nacional que contém informações detalhadas sobre:
...
1. **Importação e limpeza dos dados** brutos do CSV
2. **Tratamento de valores ausentes** (NAs)
3. **Seleção de variáveis relevantes** para análise nutricional
4. **Criação de novas variáveis** (índices nutricionais, categorias)
5. **Análise exploratória** através de visualizações e estatísticas
6. **Análise de correlações** entre alimentação, região e saúde
...
- Visualizações informativas com `ggplot2`
...
O **ENANI 2019** (Estudo Nacional de Alimentação e Nutrição Infantil) [@enani2019] foi conduzido pelo Ministério da Saúde do Brasil em parceria com universidades brasileiras. O estudo teve como objetivo:
...
### Etapa 1: Inspeção Inicial e Seleção de Variáveis
...
#### Verificação da Extração de Idade
...
### Distribuição por Região e Situação
...
**Insight**: A distribuição geográfica mostra a representatividade da amostra em todas as regiões brasileiras, com predominância urbana refletindo a realidade demográfica do país. Os dados são provenientes do ENANI 2019 [@enani2019], um estudo de base populacional representativo do Brasil.
...
## Análise de Nascimento e Saúde Perinatal
...
**Insight**: A maioria das crianças nasce com peso adequado, mas há uma proporção significativa com baixo peso ao nascer, indicando vulnerabilidade nutricional desde o início. As classificações seguem os critérios estabelecidos pela OMS [@oms2021], onde peso inferior a 2500g é considerado baixo peso e superior a 4000g caracteriza macrossomia.
...
**Insight**: A prevalência de cesarianas é notável em todas as regiões, reflexo da alta taxa brasileira de partos cirúrgicos. A classificação da idade gestacional segue os parâmetros da OMS [@oms2021], com gestações inferiores a 37 semanas consideradas prematuras e superiores a 42 semanas classificadas como pós-termo.
...
**Insight**: O aleitamento materno apresenta redução progressiva com a idade, padrão esperado mas que evidencia a necessidade de políticas de incentivo à amamentação prolongada. A OMS recomenda aleitamento materno exclusivo até os 6 meses e continuado até os 2 anos ou mais [@oms2021].

### Consumo de Alimentos Saudáveis vs Ultraprocessados
...
**Insight**: O consumo de ultraprocessados aumenta com a idade, indicando necessidade de intervenções educativas para famílias com crianças maiores. O Guia Alimentar para Crianças Brasileiras [@guia_alimentar2021] recomenda evitar alimentos ultraprocessados e priorizar alimentos in natura ou minimamente processados na alimentação infantil.

### Padrões Regionais de Alimentação
...
**Insight**: Existem diferenças regionais significativas nos padrões alimentares, com algumas regiões apresentando maior consumo de ultraprocessados.
...
<div style="text-align: center; padding: 20px; background-color: #f5f5f5; border-radius: 10px; margin-top: 30px;">
  <h3>📊 Projeto desenvolvido para a disciplina de Computação para Análise de Dados</h3>
  <p><strong>Steffano Pereira</strong> | Novembro 2025</p>
  <p style="color: #666;">Análise realizada com R, tidyverse e ggplot2</p>
</div>
